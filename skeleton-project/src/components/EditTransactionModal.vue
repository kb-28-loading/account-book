...
const props = defineProps([
  "editData",
]); /* 부모가 준 데이터 받기 MoneyListLatest의 editData */
...
// placeholder가 실제로 보이도록 초기값을 빈 문자열로 설정 (0으로 하면 placeholder가 아닌 실제 값으로 표시됨)
const userMoney = ref("");
...
const EditBtn = async () => {
  ...
  try {
    ...
    // map을 사용하여 해당 ID만 교체
    const updatedMoneyList = currentUser.moneyList.map((item) =>
